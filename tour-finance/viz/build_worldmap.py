  ...
  function draw(now){
    ...
    if(geo==="cont"){
      FLOWS.filter(vis).forEach(f=>{const cm=CONT_META[f.cont];if(!cm)return;
        ...
        ctx.save();ctx.globalAlpha=lit?0.95:0.12;ctx.fillStyle=lit?col:"#c3d0e8";ctx.shadowColor=col;ctx.shadowBlur=lit?(on?10:6):0;ctx.beginPath();ctx.arc(B[0],B[1],on?4:2.8,0,7);ctx.fill();ctx.restore();
        if(lit)txt(f.country,B[0],B[1]+12,on?"#ecedf0":"#9aa4b8","600 9px "+getFont(),2.4);});
    }
