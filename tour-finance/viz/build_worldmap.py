...
(function(){
  ...
  let view="pct",sel="all",hot=null,yi=4,hleg=null;   // yi = ดัชนีปี · hleg = ขาประเทศที่เมาส์ชี้ (โหมดทวีป)
  ...
  function litArc(f){if(hleg)return f.co===hleg.co&&f.cont===hleg.cont;if(!hot)return true;return DATA[hot]?f.co===hot:f.country===hot;}
  function litC(c){if(hleg)return c.cont===hleg.cont;if(!hot)return true;if(DATA[hot])return c.flows.some(f=>f.co===hot&&vis(f));return c.country===hot;}
  ...
  function draw(now){
    ...
    if(geo==="cont"){
      FLOWS.filter(vis).forEach(f=>{const cm=CONT_META[f.cont];if(!cm)return;
        ...
        const on=hleg===f;                                        // ขานี้ถูกเมาส์ชี้อยู่
  ...
  cv.addEventListener("pointermove",e=>{const r=cv.getBoundingClientRect(),mx=e.clientX-r.left,my=e.clientY-r.top;let f=null,nl=null;
    if(geo==="cont"){                                            // โหมดทวีป: ตรวจจุดประเทศย่อยก่อน
      FLOWS.filter(vis).forEach(x=>{const cm=CONT_META[x.cont];if(!cm)return;const A=px(cm.lat,cm.lon),B=px(x.lat,x.lon);
        if(Math.hypot(A[0]-B[0],A[1]-B[1])<6)return;if(Math.hypot(B[0]-mx,B[1]-my)<8){nl=x;f=x.co;}});
    }
    ...
    if(f!==hot||nl!==hleg){hot=f;hleg=nl;renderDetail();markHot();}cv.style.cursor=(f||nl)?"pointer":"default";});
