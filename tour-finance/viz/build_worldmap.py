...
(function(){
  ...
  const mixCols=arr=>{let r=0,g=0,b=0;arr.forEach(h=>{h=h.replace('#','');r+=parseInt(h.slice(0,2),16);g+=parseInt(h.slice(2,4),16);b+=parseInt(h.slice(4,6),16);});const n=arr.length;return `rgb(${Math.round(r/n)},${Math.round(g/n)},${Math.round(b/n)})`;};
  ...
  let view="pct",sel="all",hot=null,yi=4,hleg=null;   // yi = ดัชนีปี · hleg = ชื่อประเทศที่เมาส์ชี้ (โหมดทวีป) ไฮไลต์ทุกบริษัทที่ไปประเทศนั้น
  ...
  function markHot(){const cos=hleg?new Set(FLOWS.filter(f=>f.country===hleg&&f.thb[yi]>0).map(f=>f.co)):null;
    barsEl.querySelectorAll(".barrow").forEach(el=>el.classList.toggle("hot",cos?cos.has(el.dataset.co):el.dataset.co===hot));}
  ...
  function legDetail(country){    // hover จุดประเทศ (โหมดทวีป): ลิสต์ทุกบริษัทที่ไปประเทศนั้น + เส้นทางย้อนกลับ
    const fl=FLOWS.filter(f=>f.country===country&&f.thb[yi]>0).sort((a,b)=>fThb(b)-fThb(a));
    if(!fl.length)return overview();
    const mx=Math.max(1,...fl.map(f=>view==="thb"?fThb(f):fPct(f))),cm=CONT_META[fl[0].cont];
    dlbl.textContent="ปลายทาง (ย้อนกลับ)";
    detEl.innerHTML=`<div class="dhead">${PIN.replace('currentColor','#fbbf24')}<span class="nm">${country}</span>${cm?'<span class="chip" style="background:#33363c;color:#c3d0e8">'+cm.nm+'</span>':''}</div>`+
      `<p style="font-size:9.5px;color:var(--t3);margin:-4px 0 7px">ย้อนกลับ: ${country} → ${cm?cm.nm:''} → ไทย · ${fl.length} บริษัท</p>`+
      fl.map(f=>`<div class="drow"><div class="rl"><i class="dot" style="background:${COL[f.co]}"></i><span>${f.co}</span></div>
        <div class="rv">${view==="thb"?fmt(fThb(f)):"~"+fPct(f)+"%"}</div>
        <div class="mt"><i style="width:${(view==="thb"?fThb(f):fPct(f))/mx*100}%;background:${COL[f.co]}"></i></div></div>`).join("");
  }
  function renderDetail(){if(hleg)return legDetail(hleg);if(hot&&DATA[hot])coDetail(hot);else if(hot)countryDetail(hot);else if(sel!=="all")coDetail(sel);else overview();}
  ...
  function litArc(f){if(hleg)return FLOWS.some(x=>x.country===hleg&&x.co===f.co&&x.cont===f.cont);if(!hot)return true;return DATA[hot]?f.co===hot:f.country===hot;}
  function litC(c){if(hleg){const hf=FLOWS.find(x=>x.country===hleg);return !!hf&&c.cont===hf.cont;}if(!hot)return true;if(DATA[hot])return c.flows.some(f=>f.co===hot&&vis(f));return c.country===hot;}
  ...
  function draw(now){
    ...
    curFlows.filter(vis).forEach((f,i)=>{const P=px(f.lat,f.lon);const lit=litArc(f);const col=COL[f.co];const v=fThb(f);
      ...
      if(!reduce&&lit&&!(contMode&&!hot&&!hleg)){const sp=0.09+Math.min(0.15,v/130000);const np=(hot||hleg)&&lit?(contMode?2:4):2;
    ...
    if(geo==="cont"){
      const cd={};                                               // country -> {B,cos:[],lit,on} รวมจุดต่อประเทศ (ผสมสีถ้าหลายบริษัท)
      FLOWS.filter(vis).forEach(f=>{const cm=CONT_META[f.cont];if(!cm)return;
        ...
        const on=hleg&&f.country===hleg;                          // ขาที่ไปประเทศที่ชี้ (ทุกบริษัท)
        const lit=hleg?on:(!hot||hot===f.co||hot===cm.nm||hot===f.country),col=COL[f.co];
        // เส้นโค้งประ กางตามบริษัท (แต่ละบริษัทโค้งคนละทาง -> แยกออกว่ามาจากใคร) — วาดต่อบริษัท
        ...
        ctx.save();ctx.globalAlpha=lit?(on?0.95:0.55):0.05;ctx.strokeStyle=col;ctx.lineWidth=on?2:1.1;ctx.setLineDash([4,3]);ctx.lineCap="round";ctx.shadowColor=col;ctx.shadowBlur=on?8:0;
        ctx.beginPath();ctx.moveTo(A[0],A[1]);ctx.quadraticCurveTo(cx,cy,B[0],B[1]);ctx.stroke();ctx.restore();
        const e=cd[f.country]||(cd[f.country]={B:B,cos:[],lit:false,on:false});
        e.cos.push(f.co);if(lit)e.lit=true;if(on)e.on=true;});
      Object.keys(cd).forEach(country=>{const e=cd[country],uq=[...new Set(e.cos)];  // จุดเดียวต่อประเทศ = ผสมสีทุกบริษัท (บริษัทเดียว = สีนั้น)
        const col=uq.length>1?mixCols(uq.map(c=>COL[c]||"#8892a6")):(COL[uq[0]]||"#c3d0e8"),B=e.B,lit=e.lit,on=e.on;
        ctx.save();ctx.globalAlpha=lit?0.95:0.12;ctx.fillStyle=lit?col:"#c3d0e8";ctx.shadowColor=col;ctx.shadowBlur=lit?(on?10:6):0;ctx.beginPath();ctx.arc(B[0],B[1],on?4.4:3,0,7);ctx.fill();
        if(uq.length>1&&lit){ctx.globalAlpha=1;ctx.lineWidth=1;ctx.strokeStyle="rgba(255,255,255,.5)";ctx.stroke();}ctx.restore();  // วงขาว = หลายบริษัท
        if(lit)txt(country,B[0],B[1]+12,on?"#ecedf0":"#9aa4b8","600 9px "+getFont(),2.4);});
    }
  ...
  cv.addEventListener("pointermove",e=>{const r=cv.getBoundingClientRect(),mx=e.clientX-r.left,my=e.clientY-r.top;let f=null,nl=null;
    if(geo==="cont"){                                            // โหมดทวีป: ตรวจจุดประเทศย่อยก่อน (เก็บชื่อประเทศ -> ไฮไลต์ทุกบริษัท)
      FLOWS.filter(vis).forEach(x=>{const cm=CONT_META[x.cont];if(!cm)return;const A=px(cm.lat,cm.lon),B=px(x.lat,x.lon);
        if(Math.hypot(A[0]-B[0],A[1]-B[1])<6)return;if(Math.hypot(B[0]-mx,B[1]-my)<8)nl=x.country;});
    }
    if(!nl)COUNTRIES.forEach(c=>{if(!c.flows.some(vis))return;const P=px(c.lat,c.lon);if(Math.hypot(P[0]-mx,P[1]-my)<dotR(c)+6)f=c.country;});
    if(f!==hot||nl!==hleg){hot=nl?null:f;hleg=nl;renderDetail();markHot();}cv.style.cursor=(f||nl)?"pointer":"default";});
  cv.addEventListener("pointerleave",()=>{hot=null;hleg=null;renderDetail();markHot();});
